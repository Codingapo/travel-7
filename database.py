...
def init_db():
    ...
    _ensure_column(c, "Users", "is_admin", "INTEGER DEFAULT 0")
    _ensure_column(c, "Users", "user_type", "TEXT DEFAULT 'client'")
    ...
    c.execute("CREATE INDEX IF NOT EXISTS idx_admin_audit_created ON Admin_Audit_Log(created_at)")
    ...
    if not c.fetchone():
        # Honest placeholder until the first real sync runs - not a made-up
        # rating/count.
        c.execute(
